<div align="center">


<br />

# Hey, I'm Art

I'm a developer who likes building clean tools, Discord bots, FiveM scripts, web projects, and Windows/C++ apps.

<img src="https://komarev.com/ghpvc/?username=artlewix4&style=flat-square&label=Profile%20views&color=111111" alt="Profile views" />

</div>

---

## About me

- I work mostly on private projects and custom tools.
- I build Discord bots, FiveM scripts, web panels, and C++ Windows projects.
- I like clean UI, fast systems, and projects that feel polished.
- I usually work with JavaScript, TypeScript, Python, Lua, C++, C#, HTML, and CSS.

## Languages and tools

<div align="center">

![JavaScript](https://img.shields.io/badge/JavaScript-111111?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![TypeScript](https://img.shields.io/badge/TypeScript-111111?style=for-the-badge&logo=typescript&logoColor=3178C6)
![Python](https://img.shields.io/badge/Python-111111?style=for-the-badge&logo=python&logoColor=3776AB)
![Lua](https://img.shields.io/badge/Lua-111111?style=for-the-badge&logo=lua&logoColor=2C2D72)
![C++](https://img.shields.io/badge/C++-111111?style=for-the-badge&logo=cplusplus&logoColor=white)
![C#](https://img.shields.io/badge/C%23-111111?style=for-the-badge&logo=csharp&logoColor=9B4F96)
![HTML5](https://img.shields.io/badge/HTML5-111111?style=for-the-badge&logo=html5&logoColor=E34F26)
![CSS3](https://img.shields.io/badge/CSS3-111111?style=for-the-badge&logo=css3&logoColor=1572B6)

![Node.js](https://img.shields.io/badge/Node.js-111111?style=for-the-badge&logo=nodedotjs&logoColor=5FA04E)
![React](https://img.shields.io/badge/React-111111?style=for-the-badge&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-111111?style=for-the-badge&logo=nextdotjs&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-111111?style=for-the-badge&logo=tailwindcss&logoColor=06B6D4)
![MongoDB](https://img.shields.io/badge/MongoDB-111111?style=for-the-badge&logo=mongodb&logoColor=47A248)
![Discord.js](https://img.shields.io/badge/Discord.js-111111?style=for-the-badge&logo=discord&logoColor=5865F2)
![FiveM](https://img.shields.io/badge/FiveM-111111?style=for-the-badge&logo=fivem&logoColor=F40552)
![Visual Studio](https://img.shields.io/badge/Visual%20Studio-111111?style=for-the-badge&logo=visualstudio&logoColor=5C2D91)

</div>

## What I build

- Discord bots with commands, systems, moderation, logs, and automation.
- FiveM scripts for servers, gameplay systems, UI, and utilities.
- Web dashboards and landing pages with React, Next.js, and TailwindCSS.
- Backend APIs with Node.js, TypeScript, MongoDB, and Python.
- Windows tools and C++ projects with clean interfaces and debugging utilities.

## Projects

- **Discord bots**: custom server systems, moderation, tickets, logging, and role tools.
- **FiveM scripts**: Lua resources, server tools, UI scripts, and gameplay features.
- **Web panels**: dashboards, admin panels, and clean frontend layouts.
- **C++ tools**: Windows utilities, overlays, diagnostics, and private builds.



<div align="center">

<img src="./assets/banner.png" alt="Art frontend backend developer banner" width="100%" />

</div>
